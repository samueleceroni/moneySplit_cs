...
namespace TelegramBot.Parser.Tests
{
    ...
    public class DeleteParserTests
    {
        ...
        public void GetQueryObjectTest()
        {
            ...
            result = (new DeleteParser(TestState, WrongTestText, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsFailure);
            Assert.AreEqual(expectedError, result.Error);

            //Tests with good parameters
            var expected = (new QueryObjectBuilder() { ListName = CorrectTestText, Type = QueryType.Delete, Chat = TestChat }).Build().Value;
            result = (new DeleteParser(TestState, CorrectTestText, TestChat)).GetQueryObject();
