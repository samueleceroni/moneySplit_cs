...
namespace TelegramBot.Parser.Tests
{
    ...
    public class ResetParserTests
    {
        ...
        public void GetQueryObjectTest()
        {
            ...
            result = (new ResetParser(TestState, WrongTestText, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsFailure);
            Assert.AreEqual(expectedError, result.Error);

            //Tests with good parameters
            var expected = (new QueryObjectBuilder() { ListName = CorrectTestText, Type = QueryType.Reset, Chat = TestChat }).Build().Value;
            result = (new ResetParser(TestState, CorrectTestText, TestChat)).GetQueryObject();
