...
namespace TelegramBot.Parser.Tests
{
    ...
    public class NewTransactionParserTests
    {
        ...
        public void GetQueryObjectTest()
        {
            ...
            result = (new NewTransactionParser(TestState, WrongTestText1, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsFailure);
            Assert.AreEqual(expectedError, result.Error);

            //Tests with wrong paramenters: No value long format
            result = (new NewTransactionParser(TestState, WrongTestText2, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsFailure);
            Assert.AreEqual(expectedError, result.Error);

            //Tests with good parameters: long format
            var expected = new QueryObject(TestChat, QueryType.NewTransaction, CorrectListName, 2, String.Empty, new List<string>());
            result = (new NewTransactionParser(TestState, CorrectTestText1, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(expected.Equals(result.Value));

            //Tests with good parameters: short format
            expected = new QueryObject(TestChat, QueryType.NewTransaction, CorrectListName, 2, String.Empty, new List<string>());
            result = (new NewTransactionParser(TestState, CorrectTestText2, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(expected.Equals(result.Value));

            //Test with good parameter: description long format
            expected = new QueryObject(TestChat, QueryType.NewTransaction, CorrectListName, 2, CorrectDescription, new List<string>());
            result = (new NewTransactionParser(TestState, TestDescription1, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(expected.Equals(result.Value));

            //Test with good parameter: description sort format
            expected = new QueryObject(TestChat, QueryType.NewTransaction, CorrectListName, 2, CorrectDescription, new List<string>());
            result = (new NewTransactionParser(TestState, TestDescription2, TestChat)).GetQueryObject();
            Assert.IsTrue(result.IsSuccess);
            Assert.IsTrue(expected.Equals(result.Value));
        }
    }
